# Fix SEO tags for all travel HTML files
$travelDir = "e:\html-tools\tools\travel"

# Define per-file metadata
$filesMeta = @{
    "baggage-limit.html" = @{
...
foreach ($filename in $filesMeta.Keys) {
    $filepath = Join-Path $travelDir $filename
    if (-not (Test-Path $filepath)) { Write-Host "Not found: $filename"; continue }
    
    $meta = $filesMeta[$filename]
    $content = [System.IO.File]::ReadAllText($filepath, [System.Text.Encoding]::UTF8)
    $modified = $false
    
    # 1. Fix/add robots meta
    if ($content -notmatch 'meta name="robots"') {
        $content = $content -replace '(<link rel="canonical"[^>]+/>)', "`$1`n    <meta name=`"robots`" content=`"index, follow`" />"
        $modified = $true
        Write-Host "Added robots: $filename"
    }
    
    # 2. Add complete og:image tags if missing
    if ($content -notmatch 'og:image:width') {
        if ($content -match 'og:image"') {
            # Replace existing og:image with complete version
            $content = $content -replace '(<meta property="og:image" content="[^"]+"\s*/?>)', "`$1`n    <meta property=`"og:image:width`" content=`"1280`" />`n    <meta property=`"og:image:height`" content=`"640`" />`n    <meta property=`"og:image:type`" content=`"image/png`" />"
        } else {
            # Add og:image completely after og:locale
            $content = $content -replace '(<meta property="og:locale"[^>]+/>)', "`$1`n    <meta property=`"og:image`" content=`"https://essays4u.net/social-preview.png`" />`n    <meta property=`"og:image:width`" content=`"1280`" />`n    <meta property=`"og:image:height`" content=`"640`" />`n    <meta property=`"og:image:type`" content=`"image/png`" />"
        }
        $modified = $true
        Write-Host "Added og:image: $filename"
    }
    
    # 3. Add Twitter image if missing
    if ($content -notmatch 'twitter:image') {
        $content = $content -replace '(<meta name="twitter:description"[^>]+/>)', "`$1`n    <meta name=`"twitter:image`" content=`"https://essays4u.net/social-preview.png`" />"
        $modified = $true
        Write-Host "Added twitter:image: $filename"
    }
    
    # 4. Add complete Twitter card if missing
...
"@
        $content = $content -replace '(<!-- JSON-LD|<script type="application/ld\+json">|<link rel="preconnect")', "$twitterBlock`n`n    `$1"
        $modified = $true
        Write-Host "Added twitter card: $filename"
    }
    
    # 5. Add JSON-LD if missing
...
"@
        $content = $content -replace '(<link rel="preconnect")', "$jsonLd`n`n    `$1"
        $modified = $true
        Write-Host "Added JSON-LD: $filename"
    }
...
Write-Host "`nSEO tags fix complete!"
