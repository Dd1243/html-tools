...
/**
 * GEO (Generative Engine Optimization) 优化脚本
 * 为工具页面添加 AI 友好的结构化数据和内容
 */
...
function processToolFile(toolId, tool) {
  ...
  try {
    ...
    // 检查是否已经有 GEO 优化标记
    if (content.includes('<!-- HowTo 结构化数据 -->') ||
    ...
    // 1. 在现有的 JSON-LD 后添加 HowTo Schema
    ...
    // 2. 在主要内容区域后添加 GEO 优化内容
    // 查找 </header> 或第一个 </section> 后插入
    ...
    const geoContent = `${toolDefinition}${useCases}${faq}${updateDate}`;

    // 在 </div> 前（container 结束前）插入 GEO 内容
    // 查找最后一个 </div> 前插入
    const lastDivIndex = content.lastIndexOf('</div>');
    if (lastDivIndex !== -1) {
      content = content.slice(0, lastDivIndex) + geoContent + '\n    ' + content.slice(lastDivIndex);
    }
...
function main() {
  ...
  if (processedCount > 0) {
    console.log('\n💡 提示：');
    console.log('1. 运行 npm run format 格式化代码');
    console.log('2. 在浏览器中测试几个工具页面');
    console.log('3. 使用 Google Rich Results Test 验证结构化数据');
    console.log('4. 在 ChatGPT/Perplexity 中测试 AI 引用效果');
  }
}
