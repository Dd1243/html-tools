...
Markdown 预览适合快速确认结构，但团队协作时还要固定换行、列表缩进、代码语言标记和链接写法。提交前先用同一套预览器打开原文，检查标题层级是否跳跃、表格是否在窄屏溢出、代码块是否标注语言，以及相对链接在目标仓库中是否仍然有效。预览结果正确不代表发布平台一定一致，尤其要留意平台对脚注、HTML、任务列表和表格对齐方式的差异。正式发布前保留一份原始 Markdown，并把转换出的 HTML 放在构建目录，便于回滚和审计。
